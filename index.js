...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN?
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6289527539758:+62 895-2753-9758\n' // JANGAN KEK BABI SU
            + 'END:VCARD'
prefix = '/'
blocked = []
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Halo @${num.split('@')[0]}\nSelamat datang di group Jangan Lupa INTRO... Gsh numchar asw!!! *${mdata.subject}*`
				...
				teks = `Sayonara beban group @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6289514343919@s.whatsapp.net","629527539758@s.whatsapp.net"] // replace this with your number
			const adminbotnumber = ["6289527539758@s.whatsapp.net"]
			const frendsowner = ["62895618988866@s.whatsapp.net"]
			...
			case 'creator':
                 ...
                 client.sendMessage(from, 'Nih owner nomer owner....',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'tagme':
					...
					break
                case 'Babu':
                   client.sendMessage(from, 'Ada apa tuan?', text, {quoted: mek})
                case 'ttp':
                ...
                case 'report':
                    ...
                    client.sendMessage('6289527539758@s.whatsapp.net', options, text, {quoted: mek})
				...
				case 'loli':
				    try {
						res = await fetchJson(`https://alfians-api.herokuapp.com/api/randomloli`, {method: 'get'})
				...
				case 'nsfwloli':
				    try {
						if (!isNsfw) return reply('❌ *FALSE* ❌')
						res = await fetchJson(`https://api.afumado.net/apiv2/api/nsfw`, {method: 'get'})
						buffer = await getBuffer(res.url)
               ...
               case 'infogempa':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *BOT G GUNA Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
